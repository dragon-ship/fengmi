...
<%--    List<GoodsType> typeList = gt.All();--%>
...
<script>
    ...
    function showAdd(){
        location.href="/pl_fengmi/after/add_goods.jsp";
    }
    function checkAll(){
        var goodsids=document.getElementsByName("goodsid");
        if(allnum == 0){
    ...
    function delectAll(){
        if(confirm("确定删除吗？")){
            ...
            window.location.href="/pl_fengmi/DeleteGoodsServlet?goodsid="+ids;
        }
...
<div class="container">
    <nav aria-label="..." class="navbar-right" style="margin-right: 15px">
        <ul class="pagination">
            ...
            <li><a class=" btn-info"
                   href="<%=path %>/FuzzySelectGoods?pageNo=${pageNo}&username=${user.username}&typeId=${typeId}">${pageNo}</a></li>
            <li class="disabled"><a href="#" aria-label="Previous">»<span
                    class="sr-only">(current) </span></a></li>
            <%
                }
